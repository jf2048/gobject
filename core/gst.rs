    ...
    fn from_list(v: &[NestedMeta]) -> Result<Self, darling::Error> {
        ...
        for nmi in v {
            if let NestedMeta::Meta(syn::Meta::List(ref value)) = *nmi {
                ...
                // We replace `__` to `_%` to have a way to define pad templates
                // called "src_%u" for example
                template.name = Some(name.replace("__", "_%"));
                if template.direction.is_none() {
                    template.direction = Some(if name.to_lowercase().starts_with("src") {
                        Direction::Src
                    } else if name.to_lowercase().starts_with("sink") {
                        Direction::Sink
                    } else {
